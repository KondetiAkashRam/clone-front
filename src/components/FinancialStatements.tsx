...
export const FinancialStatements: React.FC = () => {
  ...
  const generateStatements = async () => {
    ...
    try {
      ...
      const response = await fetch('https://clone-3-jh4k.onrender.com/generate-financial-statements/', {
        method: 'POST',
  ...
  const downloadPDF = () => {
    ...
    if (activeTab === 'balance-sheet' && state.financialStatements.groupedBalanceSheet) {
      const grouped = state.financialStatements.groupedBalanceSheet;
    ...
    if (activeTab === 'profit-loss' && state.financialStatements.detailedProfitLoss && state.financialStatements.detailedProfitLoss.length > 0) {
      // Use same formatting as balance sheet/trial balance
      autoTable(doc, {
        startY: yPosition,
        head: [['Accounts', 'Amount (€)']],
        body: state.financialStatements.detailedProfitLoss.map(row => [
          row.label,
    ...
    if (activeTab === 'trial-balance' && state.financialStatements.groupedTrialBalance) {
      const grouped = state.financialStatements.groupedTrialBalance;
    ...
    if (activeTab === 'cash-flow' && state.financialStatements.cashFlow.length > 0) {
      const cashFlow = state.financialStatements.cashFlow;
      autoTable(doc, {
        startY: yPosition,
  ...
  const exportExcel = () => {
    ...
    switch (activeTab) {
      case 'balance-sheet':
        data = state.financialStatements.balanceSheet;
        sheetName = 'Balance Sheet';
        break;
      case 'profit-loss':
        data = (state.financialStatements.detailedProfitLoss || []).map(row => ({
          Accounts: row.label,
        ...
        break;
      case 'trial-balance':
        data = state.financialStatements.trialBalance;
        sheetName = 'Trial Balance';
        break;
      case 'cash-flow':
        data = state.financialStatements.cashFlow;
        sheetName = 'Cash Flow';
        break;
      default:
        return;
  ...
  const GroupedTrialBalanceTable = () => {
    ...
    // Calculate totals
    const groupTotals = {} as Record<string, {debit: number, credit: number}>;
    let grandDebit = 0;
    let grandCredit = 0;
  ...
  const GroupedBalanceSheetTable = () => {
    ...
    const groupTotals = {} as Record<string, number>;
    let grandTotal = 0;
    mainGroups.forEach(group => {
